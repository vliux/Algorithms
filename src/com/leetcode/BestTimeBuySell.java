...
public class BestTimeBuySell {
    public int maxProfit(int[] prices) {
        return _maxProfit_(prices, new Meta(), 0);
    }

    private int _maxProfit_(int[] prices, Meta parent, int i){
        System.out.println(String.format("[%d] act=%d, profit=%d", i, parent.action, parent.profit));
        if(i >= prices.length) return parent.profit;
        Meta actM = null;
        int profitAct = Integer.MIN_VALUE;
        switch (parent.action){
            case INVALID:
                actM = new Meta(parent);
                actM.profit -= prices[i];
                actM.action = BUY;
                actM.buySellDay = i;
                break;
            case BUY:
        ...
        if(null != actM) profitAct = _maxProfit_(prices, actM, i+1);

        Meta actNone = new Meta(parent);
        int profitNone = _maxProfit_(prices, actNone, i+1);
        if(profitAct > profitNone){
            //System.out.println(String.format("[%d] act=%d, profit=%d", i, actM.action, profitAct));
            return profitAct;
        }else{
            //System.out.println(String.format("[%d] act=NONE, profit=%d", i, profitNone));
            return profitNone;
        }
    }

    private static final int BUY = 101;
    private static final int SELL = 102;
    ...
    public static void main(String[] args){
        ...
        BestTimeBuySell bbs = new BestTimeBuySell();
